...
```typescript
// From src/domain/__tests__/access-state.test.ts
beforeEach(() => {
  vi.useFakeTimers();
});
...
it('returns program-window when program eligibility and window are active', () => {
  ...
  const result = computeAccessState({
    ...
    programWindowEndDate: new Date('2025-07-15T12:00:00.000Z'),
  });
...
**Fix**: Pin times to noon UTC (or another explicit fixed time in your domain model) and use `Math.ceil`/`Math.floor` explicitly rather than relying on implicit truncation.
...
If a test is hard to de-flake, that's often a sign it's too integration-y and testing too many things at once through too many layers. Before adding retries or sleeps, ask whether the test should exist in its current form.
