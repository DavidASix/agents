...
it(
  'rejects when SSO account is linked to a different user (prevents cross-user linking)',
);
it(
  'filters out records when incorrect permissions are present (enforces least privilege)',
);
